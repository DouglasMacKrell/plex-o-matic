...
class TestEpisodeIntegration:
    ...
    def test_extract_and_detect_integration(self) -> None:
        ...
        assert show_info.get("season") == 1  # Now expecting an integer
        assert show_info.get("episode") == 1  # Now expecting an integer
    ...
    def test_generate_tv_filename_with_multi_episode(self) -> None:
        ...
        filename = generate_tv_filename("Show Name", 1, 5, "Episode Title", style="dots")
        ...
        filename = generate_tv_filename(
            "Show Name", 1, [1, 3, 5], "Multi Episode", concatenated=True, style="dots"
        )
        assert "S01E01+E03+E05" in filename

        # Special characters in title
        filename = generate_tv_filename("Show: A Story", 1, [1, 2], "Episode: Part 1", style="dots")
