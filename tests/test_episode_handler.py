...
class TestMultiEpisodeDetection:
    ...
    def test_format_multi_episode_filename(self):
        ...
        filename = format_multi_episode_filename("Show", 1, [1, 3, 5], "Title", ".mp4", concatenated=True)
        assert filename == "Show.S01E01+E03+E05.Title.mp4"
            
        # Sanitize show name and title
        filename = format_multi_episode_filename("Show: The Beginning", 1, [1, 2], "Title: Part 1", ".mp4")
...
class TestSpecialEpisodeHandling:
    ...
    def test_detect_special_episodes(self):
        ...
        assert detect_special_episodes("Show.S00E01.Special.mp4") == {'type': 'special', 'number': 1}
