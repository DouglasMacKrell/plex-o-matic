...
@pytest.mark.parametrize("show_name, style, expected", [
...
@pytest.mark.parametrize("episodes, season, expected", [
    ([5], 2, 'S02E05'),
    ([1, 2, 3], 1, 'S01E01-E03'),
...
@pytest.mark.parametrize("segments, style, expected", [
    (
...
@pytest.mark.parametrize("show_name, season, episodes, title, extension, style, expected", [
    (
        'Show Name', 1, [5], 'Episode Title', '.mp4', 'dots',
        'ShowName.S01E05.Episode.Title.mp4'
    ),
    (
        'Show Name', 1, [5], 'Episode Title', '.mp4', 'spaces',
        'Show Name S01E05 Episode Title.mp4'
    ),
    (
        'Show Name', 1, [1, 2, 3], 'Multi Episode', '.mkv', 'spaces',
        'Show Name S01E01-E03 Multi Episode.mkv'
    ),
    (
        'Show Name', 1, [5], None, '.mp4', 'spaces',
        'Show Name S01E05.mp4'
    ),
    (
        'Show Name', 1, [], 'No Episodes', '.mp4', 'spaces',
        'Show Name S01E00 No Episodes.mp4'
    ),
])
...
@pytest.mark.parametrize("show_name, season, episodes, titles, extension, style, concatenated, expected", [
    (
        'Show Name', 1, [1, 2, 3],
    ...
    (
        'Show Name', 1, [1, 2, 3], 'Combined Episodes',
        '.mp4', 'dots', True,
        'ShowName.S01E01-E03.Combined.Episodes.mp4'
    ),
    (
        'Show Name', 1, [1, 2, 3], None,
        '.mp4', 'spaces', True,
        'Show Name S01E01-E03.mp4'
    ),
])
def test_format_multi_episode_filename(show_name, season, episodes, titles, extension, style, concatenated, expected):
    """Test formatting filenames for multi-episode files."""
    assert format_multi_episode_filename(show_name, season, episodes, titles, extension, style, concatenated) == expected
...
def test_construct_episode_path_with_base_dir():
    ...
    base_dir = '/media/tv'
    expected_path = os.path.join(base_dir, 'Show Name', 'Season 01', 'Show Name S01E05 Episode Title.mp4')
...
def test_construct_episode_path_with_style():
    ...
    assert construct_episode_path('Show Name', 1, [5], 'Episode Title', style='dots') == expected_path 