...
class MetadataFetcher:
    ...
    def _setup_cache(self):
        ...
        self._fetch_metadata_cached = lru_cache(maxsize=self.cache_size)(self._fetch_metadata_uncached)
            ...
            if media_type == MediaType.MOVIE:
                ...
                if 'number_of_seasons' in tv_show and tv_show['number_of_seasons'] > 0:
                    for season_number in range(1, tv_show['number_of_seasons'] + 1):
                        try:
                            season_data = self.client.get_tv_season(tv_id=tmdb_id, season_number=season_number)
                            ...
                            logger.warning(f"Error fetching season {season_number} for TV show {tmdb_id}: {e}")
